...
int main(void) {
    ...
    // ── no theme: the documented reference fallbacks, distinct per mode ──
    {
        ...
        check(dark_p.bg == as_colorref(0x282C34),
              "no theme + dark uses the reference dark bg");
        check(light_p.bg == as_colorref(0xF6F6F6),
              "no theme + light uses the reference light bg");
        check(dark_p.bg != light_p.bg, "the dark and light fallbacks differ");
        check(dark_p.record == as_colorref(0xFF3B30),
              "the record disc holds on the fallback path");
    }

    // ── the alpha lerp: endpoints and midpoint ──
    {
        check(boo_color_mix(0xFFFFFF, 0x000000, 1.0f) == 0xFFFFFF,
              "mix at alpha 1 is the foreground");
        check(boo_color_mix(0xFFFFFF, 0x000000, 0.0f) == 0x000000,
              "mix at alpha 0 is the background");
